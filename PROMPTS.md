...
## The chain (added 2026-09-03, deck version 6)

The five demos are one story. `demo.chain` says so in data:

```json
"chain": {
  "n": 2,
  "product_it": "l'assistente che scrive i documenti della casa",
  "consumes": [{ "demo": "m1", "file": "demo/catena/d1-bozze.md", "why_it": "…" }],
  "produces":  { "file": "demo/catena/d2-criteri.md", "why_it": "…" }
}
```

Rules for anyone editing this file:

- **A demo consumes a file that already exists.** The handover artefacts are
  generated by `demo-data-kit/generate_chain.py` into
  `demo-data/<settore>/demo/catena/`. Never write a beat that only works if the
  previous demo's live run succeeded.
- **`demo.files[]` is the download rail** on the slide. `path` is relative to
  the sector's pack: a path ending in `/` resolves to that folder's zip, and the
  pseudo-path `RECORDS/` resolves to the sector's records folder, whatever it is
  called. Add a file here rather than telling the speaker where to find it.
- **`demo.shape` / `shape_it`** is the demo's rhetorical shape — variance,
  judgement, ladder, build, attack. Two demos with the same shape is the bug
  this revision was written to fix. If a new demo's shape duplicates an
  existing one, change the demo, not the label.
- **`prompt.branch: "dead"`** marks a beat that produces nothing downstream.
  One per demo, at most. It is a feature: it teaches that not every run is
  useful. It is announced as dead *after* it runs.
- **`prompt.beat`** is the run order inside an ordered demo. Keep it contiguous.

## Prompts that go stale, and how to write ones that do not

A demo whose punchline is *"look, it made that up"* is a bet on model behaviour,
and that bet expires. Measured on 26/08/2026: both ChatGPT and Claude now
**refuse** to give an exact catalogue reference rather than invent one — web
search and connectors changed it. The prompt that had opened the course for a
year stopped working, silently, and the callback in M5 landed on nothing.

The design rule that came out of it:

1. **Do not ask a 2026 assistant for a fact it cannot verify.** It will decline,
   hedge, or search. Ask it to **produce a document** instead: it declines to
   *state* an unverifiable fact and cheerfully *fills* a field it was asked to
   produce. `m1-p1` works because the holes are inside a deliverable.
2. **Do not tell it the question is about a specific company.** "What scale do
   *we* use?" gets "I don't know your company". "What scale is used, answer as
   if briefing a new technician" gets the industry-average answer, confidently —
   which is a better lesson anyway, because the failure an SME actually meets is
   not invention from nothing, it is *someone else's answer wearing your words*.
3. **Prefer failures that are structural, not factual.** Retrieval returning the
   nearest wrong chunk (`m3-p5`), prose rules being overridden (`m4-p4`), a
   hidden instruction in a PDF (`m5-p1`) — these follow from how the systems are
   built, so they do not expire with the next model release.
4. **Never delete a prompt that stopped working.** Move it to `status:
   "library"` and write the date, the models tested and what changed, the way
   `m1-p4` does. The change itself is teachable material.
5. Re-test every prompt whose note contains a prediction about model behaviour
   at every T−2 dry run. They are listed in `DEMO-CHECKLIST.md` §4.
